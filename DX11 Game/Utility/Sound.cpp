...
bool Sound::InitialiseSoundFiles()
{
	...
	bool result = LoadWavFile( "Resources\\Audio\\MonoMusic.wav", &_secondaryBuffer[MAIN_MUSIC], &_secondary3DBuffer[MAIN_MUSIC] );
	if ( !result ) return false;
...
bool Sound::LoadWavFile(const char* fileName, IDirectSoundBuffer8** secondaryBuffer, IDirectSound3DBuffer8** secondary3DBuffer)
{
	int error;
	FILE* filePtr;
	unsigned int count;
