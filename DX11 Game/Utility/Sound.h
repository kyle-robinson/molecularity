...
public:
	...
	void SetMusicPause(bool isPause) { for (auto music : musicTracks) { music.second->setIsPaused(isPause); } } // Sets the isPaused for music
...
private:
	...
	bool musicOn;
	bool soundEffectsOn;
	bool masterOn; // Was soundOn, changed to master so it is less confusing
};
