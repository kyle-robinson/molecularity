...
void Input::UpdateKeyboard(const float dt)
{
	...
	if (keyboard.KeyIsPressed('E') && graphics->cameraToUse != Static && graphics->cubeInRange && (graphics->cubeHover || graphics->holdingCube))
	{
...
void Input::UpdateMouse(const float dt)
{
	while (!mouse.EventBufferIsEmpty())
	{
		...
		if (graphics->GetCube().GetEditableProperties()->GetType() == ToolType::CONVERT)
		{
			...
			{
				switch (graphics->boxToUse) //might want to refactor this and how it worked (using enums instead of strings aswell)
				{
