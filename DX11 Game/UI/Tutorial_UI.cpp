...
{
	std::vector<JSON::TextData>Gen_Text = TextLoader::Instance()->LoadText( "Gen_Text" );
	LoadedTextMap = TextLoader::Instance()->ConvertToMap( Gen_Text );

	std::vector<JSON::TextData>Movement_Controls = TextLoader::Instance()->LoadText( "Movement_Controls" );
	std::unordered_map<std::string, std::string>temp = TextLoader::Instance()->ConvertToMap( Movement_Controls );
	LoadedTextMap.insert( temp.begin(), temp.end() );

	std::vector<JSON::TextData>TOOL_TIPS = TextLoader::Instance()->LoadText( "TOOL_TIPS" );
	temp = TextLoader::Instance()->ConvertToMap( TOOL_TIPS );
	LoadedTextMap.insert( temp.begin(), temp.end() );

	std::vector<JSON::TextData>TooL_Controls = TextLoader::Instance()->LoadText( "Tool_Controls" );
	temp = TextLoader::Instance()->ConvertToMap( TooL_Controls );
	LoadedTextMap.insert( temp.begin(), temp.end() );

	std::vector<JSON::TextData>Other_Controls = TextLoader::Instance()->LoadText( "Other_Controls" );
	temp = TextLoader::Instance()->ConvertToMap( Other_Controls );
	LoadedTextMap.insert( temp.begin(), temp.end() );

	std::vector<JSON::TextData>tip_opt = TextLoader::Instance()->LoadText( "TOOL_TIPS_Options" );
...
void Tutorial_UI::GeneralTutorialText()
{
	XMVECTOR textsize;
	TextToDraw text;
...
void Tutorial_UI::MoveToNextPanel()
{
	// Wait Time
	TextToDraw text;
	text._Colour = DirectX::Colors::Black;
...
void Tutorial_UI::LoadKeyBinds( std::vector<JSON::SettingData> SettingsData )
{
	for ( auto& setting : SettingsData )
	{
		...
		{
			std::string key = std::get<std::string>( setting.Setting );
