...
/// <summary>
/// Handles the drop-down widget for the user interface.
/// </summary>
...
public:
	...
	DropDown_Widget( std::vector<ListData>DropDownList, DirectX::XMFLOAT2 size, DirectX::XMFLOAT2 pos, Colour colour, MouseData MData );
	...
	void Function( std::vector<ListData> DropDownList, DirectX::XMFLOAT2 size, DirectX::XMFLOAT2 pos, std::vector<BackgroundType> Backcolour, std::vector<ButtonBacktype> ButtonImage, XMVECTORF32 textColour, ListData Current, MouseData MData );

	ListData GetSelected() { return DataSelected; }
	void SetCurrent( int selected ) { Selected = selected; }
	int GetIntSelcted() { return Selected; }
	bool GetIsDown();
private:
	std::vector<ListData>_ListData;
	int Selected = 0;
...
template<typename ListData, typename BackgroundType, typename ButtonBacktype>
inline DropDown_Widget<ListData, BackgroundType, ButtonBacktype>::DropDown_Widget( std::vector<ListData> DropDownList, DirectX::XMFLOAT2 size, DirectX::XMFLOAT2 pos, Colour colour, MouseData MData )
{
...
template<typename ListData, typename BackgroundType, typename ButtonBacktype>
inline void DropDown_Widget< ListData, BackgroundType, ButtonBacktype>::Function( std::vector<ListData> DropDownList, DirectX::XMFLOAT2 size, DirectX::XMFLOAT2 pos, std::vector<BackgroundType> Backcolour, std::vector<ButtonBacktype> ButtonImage, XMVECTORF32 textColour, ListData Current, MouseData MData )
{
	...
	for ( uint32_t i = 0u; i < _ListData.size(); i++ )
